...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        p = str(Path(path).absolute())  # os-agnostic absolute path  补全绝对路径
        if '*' in p:  # 每次要推理几个目录
            files = sorted(glob.glob(p, recursive=True))  # glob
        elif os.path.isdir(p):  # 是否是一个目录
            files = sorted(glob.glob(os.path.join(p, '*.*')))  # dir 将目录下的每一个文件都进行输出的操作
        elif os.path.isfile(p):
            files = [p]  # files
        else:
            raise Exception(f'ERROR: {p} does not exist')

        images = [x for x in files if x.split('.')[-1].lower() in img_formats]  # 图片
        ...
        self.mode = 'image'
        if any(videos):
            self.new_video(videos[0])  # new video 视频解析
        else:
            self.cap = None
...
def img2label_paths(img_paths):
    ...
    # sa, sb = os.sep + 'JPEGImages' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings  # 我改的，需要注意
    return ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]


class LoadImagesAndLabels(Dataset):  # for training/testing 加载images和labels
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # Check cache
        self.label_files = img2label_paths(self.img_files)  # labels 图片的加载

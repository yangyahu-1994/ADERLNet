...
class Conv(nn.Module):
    # Standard convolution 卷积+标准化+激活函数
...
class Shrinkage_CW(nn.Module):
    ...
    def forward(self, x):
        x_raw = x  # 原始输入
        x = torch.abs(x)  # 绝对值(对每个元素取绝对值)
        x_abs = x  # 将绝对值赋值给x_abs变量
        x = self.gap(x)  # 对绝对值求全局平均池化
        x = torch.flatten(x, 1)  # 从第二维开始平坦化
        # average = torch.mean(x, dim=1, keepdim=True)  # CS
        average = x  # CW没有求平均
        x = self.fc(x)  # 将拉平后的x放进fc，得到alpha
        x = torch.mul(average, x)  # alpha和全局平均池化后的tensor相乘，得到阈值thresholds
        # print("thresholds: \n", x)  # 阈值不能太大，否则输出会全部为零
        # print("thresholds shape: \n", x.shape)  # 阈值的形状
        x = x.unsqueeze(2).unsqueeze(2)  # 将thresholds拓展到四维
        # 软阈值化
        sub = x_abs - x  # 绝对值减去阈值
        zeros = sub - sub  # 都是0
        n_sub = torch.max(sub, zeros)  # 返回每一列中最大值的那个元素
        y = torch.mul(torch.sign(x_raw), n_sub)  # 软阈值化的输出
        return y


class Shrinkage_CS(nn.Module):
    """
        学习通道间共享阈值的子网络
    """
    def __init__(self, channel, gap_size):
        super(Shrinkage_CS, self).__init__()
        self.gap = nn.AdaptiveAvgPool2d(gap_size)
        self.fc = nn.Sequential(
            nn.Linear(channel, channel),
            nn.BatchNorm1d(channel),
    ...
    def forward(self, x):
        ...
        x = self.gap(x)  # 对绝对值求全局平均池化
        x = torch.flatten(x, 1)  # 从第二维开始平坦化
        average = torch.mean(x, dim=1, keepdim=True)  # CS，在通道维度求平均
        # average = x  # CW
        x = self.fc(x)  # 将拉平后的x放进fc，得到alpha
        x = torch.mul(average, x)  # alpha和Average后的tensor相乘，得到阈值threshold
        x = x.unsqueeze(2).unsqueeze(2)  # 将threshold拓展到四维
        # 软阈值化
        sub = x_abs - x  # 绝对值减去阈值
        # print(sub)
        zeros = sub - sub  # 都是0
